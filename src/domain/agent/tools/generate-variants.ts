...
class GenerateVariantsExecutor {
    ...
    private async generateVariantJobs(hypothesis: Hypothesis): Promise<{ jobIds: string[]; projectId: string }> {
        ...
        // Verify project exists (quick check)
        ...
        // Quick validation - check brand analysis is available
        ...
        console.log(`[VARIANTS_TOOL] Creating 3 jobs immediately...`);
        ...
        // Start async processing - do ALL heavy work in background:
        // - DOM analysis
        // - Variant idea generation
        // - Code generation
        // - Visual refinement
        const jobProcessor = createVariantJobProcessor();
        jobProcessor.processVariantJobsInBackground(
            jobIds,
            this.projectId,
            hypothesis
        ).catch(error => {
        ...
        console.log(`[VARIANTS_TOOL] Jobs created, background processing started`);
